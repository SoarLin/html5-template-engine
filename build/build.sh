...
which coffee 1> /dev/null 2>&1 || sudo npm install -g coffee-script
display "install RequireJS in Node"
which r.js 1> /dev/null 2>&1 || sudo npm install -g requirejs
display "install bower tool ref: http://twitter.github.com/bower/"
which bower 1> /dev/null 2>&1 || sudo npm install -g bower
display "install express server ref: http://expressjs.com"
which express 1> /dev/null 2>&1 || sudo npm install -g express
display "install UglifyJS 2 node-based js compressor."
which uglifyjs 1> /dev/null 2>&1 || sudo npm install -g uglify-js
display "install sqwish node-based CSS compressor."
which sqwish 1> /dev/null 2>&1 || sudo npm install -g sqwish
display "install django-html in an HTML minifier."
which pip 1> /dev/null 2>&1 || sudo easy_install pip
which pyminify 1> /dev/null 2>&1 || sudo pip install django-htmlmin
display 'install grunt tool.'
which grunt 1> /dev/null 2>&1 || sudo npm install -g grunt-cli
display 'install mocha tool.'
which mocha 1> /dev/null 2>&1 || sudo npm install -g mocha
display 'install mocha-phantomjs tool.'
which mocha-phantomjs 1> /dev/null 2>&1 || sudo npm install -g mocha-phantomjs
display 'install gulp tool.'
which mocha-phantomjs 1> /dev/null 2>&1 || sudo npm install -g gulp
display 'install Terminal in Chrome Devtools command.'
which mocha-phantomjs 1> /dev/null 2>&1 || sudo npm install -g devtools-terminal
